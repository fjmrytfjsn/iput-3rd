...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1 * 28 * 28, 704)  # 全結合層
        self.fc2 = nn.Linear(704, 10)  # 全結合層
...
for i in range(10):
    (line[i],) = ax.plot(
        x, y[i], label=str(i)
    )  # ダミーの折れ線グラフを描いて折れ線オブジェクトlineの取得
(line2,) = ax2.plot(
    x, y[0], label="Acc", color="red"
)  # 精度のグラフの折れ線オブジェクトline2の取得
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # if batch_idx % 16 == 0:
        #     print(
        #         "epoch", epoch, "/", num_epochs, trained, "/", len(train_dataset)
        #     )  # 途中経過の表示
        #     # 評価
        ...
        #         cor = np.full(
        #             200, correct / total
        #         )  # 正解率=精度 （水平線になるように200点並べる）
        #
        #         ymax = max(
        #             list(map(lambda x: max(x), y))
        #         )  # y軸調整用に最大値(2次元)を取得
        ...
        #         ax.legend(
        #             lines, labels, loc="lower center", ncol=5
        #         )  # 左右y軸を凡例に表示する
    ...
    print(
        "Epoch: {}, Loss: {:.6f}, Time: {:.2f} sec".format(
